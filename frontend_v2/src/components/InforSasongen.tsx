 ...
 * Inför debutsäsongen.
 *
 * Schemakortet korsar SHL:s sluttabell från i fjol med hela spelprogrammet för
 * i år och visar både öppningen och säsongens tuffaste period.
 *
 * Det gäller SHL 2026/27 och ingenting annat. Väljer läsaren en spelad
 * säsong försvinner det helt — ett kort som beskriver ett annat år än det
...
export function InforSasongen({
  ...
  /** Kortet handlar om SHL 26/27. Väljer man en gammal säsong hör det inte hemma. */
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/v1/season-preview`, { cache: 'no-store' })
      ...
      .catch(() => { /* kortet är ett komplement och får utebli tyst */ });
    return () => { avbruten = true; };
  }, [aktuellSasong]);

  // Byter man till en spelad säsong beskriver "Vägen in" fel år: det är
  // 26/27:s spelprogram och ska därför bara visas för den aktuella säsongen.
  if (!aktuellSasong) return null;

  const schema = data?.schedule;
  const visaSchema = schema && !schema.error && (schema.opening?.length ?? 0) > 0;

  if (!visaSchema) return null;
